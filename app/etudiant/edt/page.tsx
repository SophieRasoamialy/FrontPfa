...
interface Edt {
  ...
  num_salle: string;
  isPresent: { present: boolean };
  id_matiere: number;
  id_niveau: number;
  heure_fin: string;
  isEntranceOnly:{entranceOnly: boolean};
}
...
const EdtPage: React.FC = () => {
  ...
  const fetchNiveaux = async (date1: Date, date2: Date) => {
    try {
      ...
      const response2 = await axios.get(
        `${process.env.NEXT_PUBLIC_BACK_URL}/api/niveaux/${response.data.id_niveau}`
      );
  ...
  const getEdts = async (niveau: string, date1: Date, date2: Date) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACK_URL}/api/edt/etudiant/${niveau}?date1=${formattedDate1}&date2=${formattedDate2}&id_etudiant=${id}`
      );
    ...
    if (faceMatch) {
      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_BACK_URL}/api/pointages`,
          {
            id_edt: idEdt,
            id_etudiant: id,
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          Swal.fire(
            "Succès",
            "Merci de faire votre pointage.\n N'oublier pas de faire le pointage avant de sortir apre le cours",
            "success"
          );
        ...
        setCapturedImage(null);
        console.error(
          "Erreur lors de la création ou de la mise à jour du pointage",
          error
        );
        throw error;
      }
        ...
        if (imageSrc) {
          ...
          try {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_BACK_URL}/api/etudiants/${id}/photo`
            );
          ...
          // Vérifier que imageSrc n'est pas null avant de l'utiliser dans fetch
          if (imageSrc) {
            ...
            console.error("L'image capturée est nulle");
            return false;
          }
        }
      }
  ...
  const findNearestClass = () => {
    ...
    const nearestClass = edts.reduce<{
      ...
      if (
        difference > 0 &&
        (nearest === null || difference < nearest.difference)
      ) {
  ...
  if (nearestClass) {
    days = Math.floor(nearestClass?.difference / (24 * 60));
    hours = Math.floor((nearestClass?.difference % (24 * 60)) / 60);
    minutes = nearestClass?.difference % 60;
  }
  ...
  const heureMinuteActuelle = `${heureActuelle
    .toString()
    .padStart(2, "0")}:${minuteActuelle
    .toString()
    .padStart(2, "0")}:${secondeActuelle.toString().padStart(2, "0")}`;
  ...
  const debutSemaineFormat = debutSemaine.toLocaleDateString("fr-FR", {
    month: "long",
    day: "numeric",
  });
  const finSemaineFormat = finSemaine.toLocaleDateString("fr-FR", {
    month: "long",
    day: "numeric",
  });
  ...
  return (
    <div className="w-full bg-gradient-to-br from-teal-100 to-white p-5">
      <div>
        <h3 className="text-center m-5 font-bold text-2xl text-teal-600">
          Emploie du temps {niveaulbl}
        </h3>
      </div>
        ...
        <div className="wrapper bg-white bg-opacity-70 rounded-2xl shadow w-full">
          <div className="header flex justify-between border-b p-2">
            <span className="text-lg font-bold text-teal-600">
              Semaine du {debutSemaineFormat} - {finSemaineFormat},{" "}
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="bg-teal-200">
                {["Heure", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                  (day, index) => (
                    <th
                      key={index}
                  ...
                  <tr className="text-center h-20">
                    ...
                    {["Mon", "Tue", "Wed", "Thu", "Fri"].map(
                      (day, dayIndex) => (
                        <td
                          key={dayIndex}
                          className="border-r p-2 xl:w-40 lg:w-30 md:w-30 sm:w-20 w-10"
                        >
                          {edts.map((edt) => {
                            ...
                            if (
                              ...
                              const startHour = parseInt(
                                hourRangeArray[0].split(":")[0]
                              );
                              const endHour = parseInt(
                                hourRangeArray[1].split(":")[0]
                              );
                              const heureEdt = parseInt(
                                edt.heure.split(":")[0]
                              );
                              const edtHeure = parseTime(edt.heure);
                              const edtHeureFin = parseTime(edt.heure_fin);
                              const formattedTime =
                                parseTime(heureMinuteActuelle);
                              if (heureEdt >= startHour && heureEdt < endHour) {
                                return (
                                  <div key={edt.id_edt} className="text-left">
                                    <div className="text-sm font-bold">
                                      {edt.matiere} {edt.id_edt}
                                    </div>
                                    <div className="text-xs">
                                      {" "}
                                      Prof: {edt.nom_enseignant}{" "}
                                      {edt.prenom_enseignant}
                                    </div>
                                    <div className="text-xs">
                                      {" "}
                                      Salle: {edt.num_salle}
                                    ...
                                    <div className="text-xs">
                                      {edt.date < dateFormate ? (
                                        ...
                                        <div>
                                          {edtHeure && edtHeureFin && formattedTime && edtHeure.getTime() <
                                            ...
                                            <div>
                                              {edt.isEntranceOnly
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() =>
                                                    openModalSortie(edt.id_edt)
                                                  }
                                                  className=" items-center justify-center text-gray-900 bg-gradient-to-r from-teal-300 to-lime-300 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-300 focus:ring-4 focus:outline-none focus:ring-lime-300  rounded-lg text-xs px-2 py-2 mx-auto "
                                                >
                                                  Pointage(sortie)
                                                </button>
                                              ) : (
                                          ...
                                          ) : edtHeure && formattedTime && edtHeure.getTime() >
                                          formattedTime.getTime() ? (
                                            <div>
                                              <button
                                                type="button"
                                                className=" opacity-50 items-center justify-center cursor-not-allowed flex text-gray-900 bg-gradient-to-r from-teal-300 to-lime-300 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-300 focus:ring-4 focus:outline-none focus:ring-lime-300  rounded-lg text-xs px-2 py-2 mx-auto "
                                                disabled
                                              >
                                                Pointage(entrée)
                                              </button>
      ...
      <div>
        {nearestClass && nearestClass.difference !== undefined && (
          <div className="max-w-sm p-6 mx-auto my-5 bg-white border bg-opacity-70 border-gray-200 rounded-3xl shadow hover:bg-gray-100">
            <p className="font-normal text-gray-900">
              Prochain cours dans :{" "}
              {nearestClass?.difference !== undefined ? (
                <span className="text-gray-900 font-bold">
                  {days !== undefined && days > 0 ? `${days} jour${days > 1 ? "s" : ""} ` : ""}
                  {hours !== undefined && hours > 0 ? `${hours} heure${hours > 1 ? "s" : ""} ` : ""}
                  {minutes !== undefined && minutes > 0
                    ? `${minutes} minute${minutes > 1 ? "s" : ""}`
                    : ""}
                </span>
              ) : (
                // Handle the case when nearestClass.difference is undefined
                <span>Aucun prochain cours</span>
              )}
            </p>
          </div>
        )}
