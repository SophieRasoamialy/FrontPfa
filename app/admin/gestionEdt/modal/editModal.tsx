...
const EditModal: React.FC<ModalProps> = ({ isOpen, onClose, edtId }) => {
  ...
  const fetchEdt = async () => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACK_URL}/api/edt/id/${edtId}`
      );
  ...
  const fetchMatiere = async (niveau: number) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACK_URL}/api/matieres/niveau/${niveau}`
      );
  ...
  const handleEnregistrer = async () => {
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BACK_URL}/api/edt/${edtId}`,
        {
          date: selectedDate,
          heure: heureDeb,
          heure_fin: heureFin,
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md sm:max-w-lg md:max-w-xl">
        ...
        <div>
          <form className=" rounded-lg ">
            ...
            <div className="mb-4">
              ...
              <input type="time" value={heureDeb} onChange={handleHeureDeb} />
              <label
                htmlFor="prof"
                className="block text-sm font-medium text-gray-900"
              >
              ...
              <input type="time" value={heureFin} onChange={handleHeureFin} />
            </div>
            <div className="mb-4">
              <label
                htmlFor="prof"
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button
            ...
            type="button"
            className=" flex text-gray-900 bg-gradient-to-r from-teal-300 to-lime-300 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-300 focus:ring-4 focus:outline-none focus:ring-lime-300 font-medium rounded-lg text-sm px-4 py-2 float-right"
          >
            Enregistrer
          </button>
        </div>
