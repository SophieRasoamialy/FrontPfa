...
interface ModalProps {
  ...
  const handleEnregistrer = async () => {
    try {
      const response = await axios.post('http://localhost:3001/api/edt', {
        date:selectedDate,
        heure:heureDeb,
        heure_fin:heureFin,
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white rounded-lg p-8">
          ...
          <button onClick={onClose} className=" bg-red-500 text-white px-4 py-2 rounded-lg font-medium text-sm">
            Close
          </button>
...
function setSelectedEnseignant(selectedValue: any) {
